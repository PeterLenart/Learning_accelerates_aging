...
}, landscape::{generate_grids, compute_fitness_grid_parallel_lmax, compute_fitness_grid_parallel_gmax, compute_fitness_grid_point_lmax, compute_fitness_grid_point_gmax, compute_survival_grid_parallel}};
...
fn main() -> Result<(), Box<dyn Error>>{
    ...
    // let learning_parameters = [25.0, f64::INFINITY, 0.0];
    ...
    let fitness_base_b = compute_survival_grid_parallel(&base_b_grid, &aging_intermediate_closure, &aging_parameters, &learning_parameters, &growth_parameters);
    
    // Save result
    // let output_file_name = "./output/toy_model_landscapes/csv/fitness_40_50.csv";
    // Name the output file based on the parameters
    let output_file_name = format!("./output/toy_model_landscapes/csv/survival_no_improvement.csv");
    ...
    // Write headers
    let _ = writer.write_record(&["b", "age", "survival"]);
