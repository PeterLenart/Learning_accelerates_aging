...
pub fn compute_survival_grid_point(grid_point: Vec<f64>, aging_intermediate_closure: &dyn Fn(f64, &[f64], &[f64], &[f64]) -> f64, aging_parameters: &[f64], learning_parameters: &[f64], growth_parameters: &[f64]) -> f64 {
    let new_aging_parameters: Vec<f64> = match aging_parameters.len() {
        1 => vec![grid_point[0]],
        2 => vec![aging_parameters[0], grid_point[0]],
        _ => {
    ...
    let age = grid_point[1];
    survival_l(age, aging_intermediate_closure, &new_aging_parameters, &learning_parameters, &growth_parameters)
}
...
pub fn compute_survival_grid_parallel<F>(
    grid: &Array2<f64>,
    aging_intermediate_closure: &F,
    aging_parameters: &[f64],
    learning_parameters: &[f64],
    ...
    F: Fn(f64, &[f64], &[f64], &[f64]) -> f64 + Send + Sync,
{
    ...
    let mut results: Vec<(usize, f64)> = grid.axis_iter(ndarray::Axis(0))
        ...
        .par_bridge()
        .map(|(index, row)| {
            let res = compute_survival_grid_point(
                row.to_vec(),
                aging_intermediate_closure,
                aging_parameters,
                learning_parameters,
